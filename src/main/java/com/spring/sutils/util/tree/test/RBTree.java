package com.spring.sutils.util.tree.test;


/**
 * @Auther :huiqiang
 * @Description :
 * @Date: Create in 上午9:57 2018/6/21 2018
 * @Modify:
 */
...
//      return search2(root, key); //使用递归的方法，本质一样的
    ...
    private void remove(RBNode<T> node) {
        ...
        if((node.left != null) && (node.right != null)) {
            //应该可以进行直接的对replace和node进行一个互换交换key值应该就可以// ，然后处理replace父子节点的关系
    ...
    private void removeFixUp(RBNode<T> node, RBNode<T> parent) {
        ...
        while((node == null || isBlack(node)) && (node != this.root)) {
            if(parent.left == node) {
                //node是左子节点，下面else与这里的刚好相反，一种情况是猴急节点是她的删除的节点的直接子节点会出现当前节点是在右边的情况
